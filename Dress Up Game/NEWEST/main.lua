...
local hitArea = display.newRect( display.contentCenterX, 500, 200, 200 )
hitArea:setFillColor( 0, 1, 1, 0.25 )

local hitArea2 = display.newRect( display.contentCenterX, 725, 200, 200 )
hitArea2:setFillColor( 0, 1, 0, 0.25 )
...
local function timerCount()
    ...
    if ( endX == startX ) and ( endY == startY ) and ( touching == true ) then
        touchCommand = "dragMe"
        currentIcon:setFillColor( 1, 1, 1, 0.5 )
...
local function buttonPress( event )
    ...
    elseif ( event.target.isFocus ) then
        if ( event.phase == "moved" ) then  
            ...
            if ( touchCommand == "dragMe" ) or ( event.target.group == display.currentStage ) then
                myText.text = "Drag and drop"

                -- START BUTTON DRAG:
                display.currentStage:insert( event.target )
                event.target.group = display.currentStage
                event.target:toFront()
                --event.target:setFillColor( 1, 1, 1, 0.5 )
        ...
        elseif ( event.phase == "ended" or event.phase == "cancelled" ) then
            ...
            elseif ( event.target.group == display.currentStage ) then
                if ( hasCollided( event.target, hitArea )) then
                    transition.to( event.target, { time=50, x=hitArea.x, y=hitArea.y } )
                else
